...
function DashboardLaunchGrid({
  ...
  return (
    <div className="dashboard-today">
      ...
      <section className="dashboard-today-plan"><div className="dashboard-plan-intro"><div><p className="eyebrow">Today on Quizo</p><h2>Your {todayMinutes}-minute session</h2><p>Based on {focusLabel}</p></div><div className="dashboard-time-picker" aria-label="Choose session duration">{([5, 10, 15, 30] as const).map((minutes) => <button className={todayMinutes === minutes ? 'active' : ''} key={minutes} onClick={() => chooseTodayMinutes(minutes)} type="button">{minutes}<small>min</small></button>)}</div></div><div className="dashboard-plan-body"><div className="dashboard-plan-list">{todayPlan.map((item, index) => <article className={item.tone} key={item.id}><span>{item.icon}</span><div><small>{index === 0 ? 'Start here' : 'Then'}</small><strong>{item.title}</strong><p>{item.detail}</p></div><b>{item.minutes} min</b></article>)}</div><aside><div><small>Today’s plan</small><strong>{todayPlan.length} activit{todayPlan.length === 1 ? 'y' : 'ies'} · {plannedMinutes} min</strong></div><button className="primary-button" onClick={nextTodayItem.action} type="button"><Play size={17} /> Start</button></aside></div></section>
