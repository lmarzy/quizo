...
begin
  ...
  if game_record.current_member_id is not null and game_record.current_member_id <> p_member_id then
    raise exception 'Another player has the second chance';
  end if;
  ...
  next_attempt := case when game_record.current_member_id is null then 1 else 2 end;
  ...
  values (
    ...
    member_record.display_name ||
      case
        when answer_correct and next_attempt > 1 then ' recovered and gained ' || points_delta || ' points'
        when answer_correct then ' answered correctly and gained ' || points_delta || ' points'
        when next_attempt = 1 then ' answered incorrectly and gets a second chance'
  ...
  next_locked_member_id := case
    when not answer_correct and next_attempt = 1 then member_record.id
    else null
  end;
  next_attempt := case when next_locked_member_id is null then 1 else 2 end;
  ...
  update public.games
  set current_member_id = next_locked_member_id,
  ...
  values (
    game_record.id,
    next_locked_member_id,
    'speed_round_started',
    case
      when next_locked_member_id is null then 'Next speed round started'
      else member_record.display_name || ' has a second chance'
    end,
    jsonb_build_object('question_id', next_question.id, 'round_number', next_round_number, 'locked_member_id', next_locked_member_id)
  );
